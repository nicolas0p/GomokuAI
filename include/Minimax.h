...
class Minimax : public InputComponent {
	...
	private:
		...
		std::pair<std::pair<int, int>, int> step(Board * board, unsigned int depth, const std::function<bool (const int&, const int&)>& compare, const Board::Moves& player, int pruning_factor);
